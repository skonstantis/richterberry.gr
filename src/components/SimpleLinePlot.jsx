...
export function SimpleLinePlot({ samples, getVirtualTimeNow, width = 600, height = 200 }) {
  ...
  if (!samples || !samples.length) {
    return <div style={{ width, height, border: "1px solid black", display: "flex", alignItems: "center", justifyContent: "center" }}>
      No data to display
    </div>;
  }

  let virtualTimeNow = getVirtualTimeNow();
  ...
  const visibleSamples = samples.filter(s => s.timestamp >= xMin && s.timestamp <= xMax);
  ...
  const points = visibleSamples.map(s => `${scaleX(s.timestamp)},${scaleY(s.value)}`).join(" ");
